class OsTypesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @os_type.save
        format.html { redirect_to os_types_url, notice: 'OS type was successfully created.' }
  ...
  def update
    respond_to do |format|
      if @os_type.update(os_type_params)
        format.html { redirect_to os_types_url, notice: 'OS type was successfully updated.' }
  ...
  private
    ...
    def os_type_params
      params.require(:os_type).permit(:name)
    end
end
